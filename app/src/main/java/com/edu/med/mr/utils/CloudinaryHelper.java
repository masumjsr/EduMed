...
public class CloudinaryHelper {
    public static String uploadImageResource(Resource resource) {
        UploadRequest request = MediaManager.get().upload(Uri.parse(resource.getLocalUri()))
                ...
                .policy(MediaManager.get().getGlobalUploadPolicy().newBuilder().maxRetries(2).build()).callback(new UploadCallback() {
                    @Override
                    ...
                    public void onProgress(String requestId, long bytes, long totalBytes) {
                        Log.i("123321", "Process");
                    }

                    @Override
                    public void onSuccess(String requestId, Map resultData) {
                        Log.i("123321", "success:"+resultData.get("uri"));
                    }
